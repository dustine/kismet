...
class RenderTargetSlot {
    ...
    void drawTargetSlot(SlotTarget slot) {
        int x = slot.getIconOrigin().x;
        int y = slot.getIconOrigin().y;
...
//            this.itemRender.renderItemAndEffectIntoGUI(this.mc.thePlayer, itemstack, i, j);
            renderItemAndEffectIntoGUI(minecraft.thePlayer, stack, slot.getIconSize(), x, y);
    ...
    /**
     * Code from {@link net.minecraft.client.renderer.RenderItem} Beyond adding the size, did some 1.8 lambda instant
     * transforms that I'll revert later when I port it back to 1.7 or so
     */
    private void renderItemAndEffectIntoGUI(EntityPlayerSP player, ItemStack stack, int size, int x, int y) {
        if (stack != null && stack.getItem() != null) {
            this.guiDisplay.setZLevel(this.guiDisplay.getZLevel() + 50.0F);

            try {
                renderItemModelIntoGUI(stack, size, x, y,
     ...
     * factor
     */
    private void renderItemModelIntoGUI(ItemStack stack, int size, int x, int y, IBakedModel model) {
        ...
        setupGuiTransform(size, x, y, model.isGui3d());
     ...
     */
    private void setupGuiTransform(int size, int x, int y, boolean isGui3d) {
        GlStateManager.translate((float) x, (float) y, 100.0F + this.guiDisplay.getZLevel());
        GlStateManager.translate(size / 2.0, size / 2.0, 0.0F);
